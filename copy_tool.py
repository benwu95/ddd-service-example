...
def init():
    ...
    with open(root.joinpath('Makefile'), 'r') as fin, open(cwd.joinpath('Makefile'), 'w') as fout:
        content = fin.read()
        content = content.replace('ddd-service-template', cwd.name)
        fout.write(content)
    with open(root.joinpath('app/config.py'), 'r') as fin, open(cwd.joinpath('app/config.py'), 'w') as fout:
        content = fin.read()
        content = content.replace('ddd-service', service_name)
        content = content.replace('ddd_service', service_name.replace('-', '_'))
        fout.write(content)
    with open(root.joinpath('app/message_queue_consumer.py'), 'r') as fin, open(cwd.joinpath('app/message_queue_consumer.py'), 'w') as fout:
        content = fin.read()
        content = content.replace('ddd-service', service_name)
        fout.write(content)
    with open(root.joinpath('app/port/restful/openapi/api.yml'), 'r') as fin, open(cwd.joinpath('app/port/restful/openapi/api.yml'), 'w') as fout:
        content = fin.read()
...
def add(name: str):
    ...
    with open(cwd.joinpath('app/adapter/repository/orm/__init__.py'), 'a') as f:
        f.write(f'from .{name}_model import {snake_to_pascal_case(name)}Model, {snake_to_pascal_case(name)}ArchiveModel\n')
