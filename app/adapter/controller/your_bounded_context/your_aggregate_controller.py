...
from app.core.your_bounded_context.use_case.your_aggregate_use_case import YourAggregateUseCase
...
class YourAggregateController(ControllerBase):
    ...
    async def get_your_aggregate(
        self,
        get_request: GetYourAggregateRequest
    ) -> YourAggregateResponse:
    ...
    async def search_your_aggregates(
        self,
        search_request: SearchYourAggregatesRequest
    ) -> SearchYourAggregatesResponse:
    ...
    async def export_your_aggregates(
        ...
        return create_xlsx(
            title='YourAggregates',
            dc_type=YourAggregateExcel,
            dc_list=data
        )
    ...
    async def delete_your_aggregate(
        self,
        *delete_requests: DeleteYourAggregateRequest
    ) -> list[str] | str:
        resp = []
        for delete_request in delete_requests:
            await self.use_case.delete_your_aggregate(
                delete_request.id,
                delete_request.doer
            )
    ...
    async def update_your_aggregate(
        self,
        *update_requests: UpdateYourAggregateRequest
    ) -> list[str] | str:
        resp = []
        for update_request in update_requests:
            await self.use_case.update_your_aggregate(
                update_request.id,
                update_request.your_value_object,
                update_request.doer
            )
    ...
    async def void_your_aggregate(
        self,
        *void_requests: VoidYourAggregateRequest
    ) -> list[str] | str:
        resp = []
        for void_request in void_requests:
            await self.use_case.void_your_aggregate(
                void_request.id,
                void_request.doer
            )
