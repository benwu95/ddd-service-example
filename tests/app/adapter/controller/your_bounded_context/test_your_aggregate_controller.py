...
async def created_your_aggregate_id():
    ...
    create_request = CreateYourAggregateRequest.create_strictly(
        your_value_object={"property_a": "value1", "property_b": 123},
...
async def test_search_your_aggregates_by_search_key_fields_startswith(
    created_your_aggregate_id,
):
    controller = YourAggregateController()
    request = SearchYourAggregatesRequest.create_strictly(
        ids=[created_your_aggregate_id],
...
async def test_search_your_aggregates_by_search_key_fields_endswith(
    created_your_aggregate_id,
):
    controller = YourAggregateController()
    request = SearchYourAggregatesRequest.create_strictly(
...
async def test_search_your_aggregates_by_search_key_fields_equals(
    created_your_aggregate_id,
):
    controller = YourAggregateController()
    request = SearchYourAggregatesRequest.create_strictly(
...
async def test_update_your_aggregate(test_db_session, created_your_aggregate_id):
    your_value_object = {"property_a": "value2", "property_b": 321}
...
async def test_void_your_aggregate(test_db_session, test_rabbitmq, created_your_aggregate_id):
    ...
    queue_watcher.assert_message_published(
        QueueMessage(
            "test-trace-id",
            YourAggregateServiceFuntion.YOUR_AGGREGATE_VOIDED,
            [YourAggregateVoided(your_aggregate_id)],
        )
    )
    queue_watcher.close()
